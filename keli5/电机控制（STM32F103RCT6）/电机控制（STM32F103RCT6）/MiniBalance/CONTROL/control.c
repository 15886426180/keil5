...
void Kinematic_Analysis(float velocity,float angle)
{
		...
		Target_Right=velocity*(1+T*tan(angle)/2/L);      //后轮差速
	...
	static float Bias,Last_Bias;  //偏差和历史值
			if(Flag_Way==0)//串口等控制方式
			{
					 if(CAN_ON_Flag==0&&Usart_ON_Flag==0) 
						{
							if(Flag_Direction==0||Flag_Direction==11) Velocity=0,Angle=0;   //停止
							else if(Flag_Direction==1) Velocity=RC_Velocity,Angle=0;        //前进
							else if(Flag_Direction==2) Velocity=RC_Velocity,Angle=PI/4;     //右前
							else if(Flag_Direction==3) Velocity=0,Angle=0;        //无控制
							else if(Flag_Direction==4) Velocity=-RC_Velocity,Angle=PI/4;   // 右后
							else if(Flag_Direction==5) Velocity=-RC_Velocity,Angle=0;      //后退
							else if(Flag_Direction==6) Velocity=-RC_Velocity,Angle=-PI/4;  //左后
							else if(Flag_Direction==7) Velocity=0,Angle=0;       //无控制
							else if(Flag_Direction==8) Velocity=RC_Velocity,Angle=-PI/4;   //左前
							else if(Flag_Direction==9) Velocity=0,Angle=-PI/2.5;   //舵机左转
							else if(Flag_Direction==10) Velocity=0,Angle=PI/3;   //舵机右转
							else Velocity=0,Angle=0;   //停止
						}
						 else
					 {
						 if(rxbuf[1]==0)Velocity=rxbuf[0]; //识别运动方向
						 else           Velocity=-rxbuf[0]; //速度
			...
			else	if(Flag_Way==3)//电磁巡线
		{
			 ...
			 Angle=-myabs(Bias)*Bias*0.0002-Bias*0.001-(Bias-Last_Bias)*0.05; //非常规PID公式
