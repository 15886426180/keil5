...
void CAN1_SEND(void) 
{
        ...
        Temp_GZ=Gryo_Z+32768;//角速度数据处理
			  ...
			  txbuf[0]=abs(Encoder_Left);//左电机速度
				txbuf[1]=Direction_Left;		//左电机方向
				txbuf[2]=abs(Encoder_Right);	//右电机速度	
				txbuf[3]=Direction_Right;//右电机方向
				txbuf[4]=Temp_Roll>>8;//前进方向角度数据高8位	
				txbuf[5]=Temp_Roll&0x00ff;//前进方向角度数据低8位
				txbuf[6]=Temp_GZ>>8;	//陀螺仪数据高8位	
			  txbuf[7]=Temp_GZ&0x00ff;	//陀螺仪数据低8位
